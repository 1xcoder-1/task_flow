...
export const TagBadge = ({ name, color, size = "sm", variant = "default", isActive }: TagBadgeProps) => {
  ...
  if (variant === "filter") {
    return (
      <span
        ...
        className={`inline-flex items-center font-medium text-white transition-all rounded-full border shadow-2xs ${
          isSm ? "text-xs px-2.5 py-0.5" : "text-xs px-3 py-1 tracking-wide"
  ...
  return (
    <span
      ...
      className={`inline-flex items-center font-bold rounded-full border shadow-2xs transition-all ${
        isSm ? "text-xs px-2.5 py-0.5" : "text-xs px-3 py-1 tracking-wide"
      }`}
    >
      <span
